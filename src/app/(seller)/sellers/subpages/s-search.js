                            ...
                            <div className="fx1 bg-[#fbfbfb]_ oy mt-3 space-y-4 text-md px-1">
                                {MOCK_PARTNERS.map(v =>
                                    <div className="hover:bg-[#fbfbfb] cursor-pointer px-3 border_ rounded-lg border-blue-400 space-y-1">
                                        <h1 className="text-blue-600 font-semibold">{v.name}</h1>
                                        <div className="text-green-600 text-sm">Bussniess Type: <span className="font-semibold">{v.type}</span></div>
                                        <div className="text-sm text-gray-600 text-sm">{v.Description}</div>
                                        <div className="df aic  gap-3 text-xs"><span className="df aic gap-1"><MapPin size={12}/>{v.location}</span><div>{v.tags.map(t=><span className="px-2 bg-gray-100">{t}</span>)}</div></div>
                                    </div>
                                )}
                            </div>
                        </div>
