...
// --- Header Component ---
export default function Header({ elements, variant = "default" }) {
  return (
    <header className="">
      <div className={"mx-auto px-4 sm:px-6 lg:px-8 p-2 flex justify-between items-center" + (variant !== "default" &&" bg-black text-white")}>
        {/* Logo and Brand Name */}
        ...
        <nav className="sm:flex space-x-8 text-sm font-medium">
          {elements.length > 0 && elements.map((v, i) => (<div key={i}>{v}</div>))}
        </nav>
      </div>
    </header>
  )
