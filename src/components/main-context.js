        ...
        <>
            {device && <div className="df fd-c hfp_">
                ...
                <dynamic_.Provider value={{ device, user }}>
                    ...
                    {["/messages", "/form"].every(v => v !== pathname) && <Topbar_ topbar={<Topbar />} />}

                    <main className="" ref={mainRef} >
                        {children}
                    </main>

                    <lowscreen-nav className="flex md:hidden bdt pS bottom-0 z-50" id="topbar" style={{ background: '#fafafa', fontSize: '0.75rem', ...(false ? { paddingInlineStart: 'calc(0.5rem + 1vmin)' } : { paddingInline: 'calc(0.5rem + 1vmin)' }) }}>
